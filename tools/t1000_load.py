...
class data_loader():
    def __init__(self, data_path,  model= 'train' , time=1, transform=None):
        self.data_path = data_path
        
        if model == 'train':
              self.path_file  = data_path+'split/train'+str(time)+'.txt'
        elif model == 'test1':
              self.path_file = data_path+'split/t821_test'+str(time)+'.txt'
        elif model == 'test2':
              self.path_file = data_path+'split/t1000_test'+str(time)+'.txt'      
        else:
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        thermal = np.array(thermal).astype(np.float32).transpose((2, 0, 1)) / 255.0
