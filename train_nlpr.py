...
import tools.nprl_load as data_loader
...
def train_iter4(net, train_data_loader, device, optimizer, criterion):
    train_loss = 0.0
    net.train()
    hard = -1
    rgb_h = None
    ...
    for i_batch, sample_batched in enumerate(train_data_loader):
            ...
            ##################获取结果##################
            if hard > -1:
                rgb = torch.cat((rgb, rgb_h), 0)
                thermal = torch.cat((thermal, depth_h), 0)
                gt = torch.cat((gt, gt_h), 0)
            optimizer.zero_grad()
            out = net(rgb, thermal)
            ##################计算损失###################
            loss, l_re, hard = criterion(out, gt)
            ###################反向传播##################
            loss.backward()
            optimizer.step()
            ############每个batch的结果统计和输出##########
            ...
            if i_batch % 100 == 0 and i_batch > 0:
                ...
                print('step:' , i_batch, 
                      ...
                      lstr)
            if hard > -1:
                rgb_h = rgb[hard:hard+1,:,:,:]
                depth_h = thermal[hard:hard+1,:,:,:]
                gt_h =  gt[hard:hard+1,:,:,:]      
    return train_loss / i_batch
...
###############################################################################
#####################读取数据###################################################
    Date_File = "/media/hpc/data/work/dataset/NPRL/"
    
    Date_File = "/data/HNC/dataset/RGBD_Saliency/NPRL/"
...
def get_trianer(args, model_tpye = 0):      
  ...
  if model_tpye == 4:
    train(args, train_iter4, val_iter, datsets=1)  
  if model_tpye == 5:
    train(args, train_iter4, val_iter, datsets=2)  
  if model_tpye == 6:
    train(args, train_iter4, val_iter, datsets=3)  
