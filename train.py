...
#训练集
import tools.train_loader as train_loader
#测试集
import tools.nju_load as nju_loader
import tools.nprl_load as nprl_loader
...
###############################################################################
#####################读取数据###################################################
    Date_File = "/media/hpc/data/work/dataset/"
    image_h, image_w = train_loader.get_Parameter()
    print('=>1: 输入图像参数', image_h, image_w)
#####################读取训练数据###################################################
    train_dataset = train_loader.data_loader(Date_File,'train', subset=datsets,
                               transform=transforms.Compose([
                                           train_loader.scaleNorm(image_w, image_h, (1, 1.2), True),
                                           train_loader.RandomRotate(10),
                                           train_loader.RandomCrop(image_w, image_h),
                                           train_loader.RandomFlip(),
                                           train_loader.RandomGaussianBlur(),
                                           train_loader.RandomAdjust(),
                                           train_loader.ToTensor(),
                                           ]))
    train_data_loader = DataLoader(train_dataset, args.batch_size,shuffle=True, num_workers=4)

#####################读取测试数据###################################################
    nju_val_dataset = nju_loader.data_loader(Date_File+'nju2000/','test',subset=datsets,
                                    transform=transforms.Compose([
                                       nju_loader.scaleNorm(image_w, image_h, (1, 1.2), False),
                                       nju_loader.ToTensor(),
                                       ]))
    nju_val_data_loader = DataLoader(nju_val_dataset, 1, shuffle=False, num_workers=2)
###############################################################################
    nset = datsets
    if datsets == 4:
        nset = 1
    nprl_val_dataset = nprl_loader.data_loader(Date_File+'NPRL/','test',subset=nset,
                                    transform=transforms.Compose([
                                       nprl_loader.scaleNorm(image_w, image_h, (1, 1.2), False),
                                       nprl_loader.ToTensor(),
                                       ]))
    nprl_val_data_loader = DataLoader(nprl_val_dataset, 1, shuffle=False, num_workers=2)
...
##############################开始训练##########################################
    ...
    for epoch in range(int(args.start_epoch), args.epochs):
        ...
        if epoch % args.val_freq ==0:
            ...
            val_loss, mae, maxF, Sm = val_fun(net, nju_val_data_loader, device, L1_loss, eval_tool)
            totall_time =  time.time()-begin_time
...
def get_trianer(args, model_tpye = 0):      
  if model_tpye == 1:
