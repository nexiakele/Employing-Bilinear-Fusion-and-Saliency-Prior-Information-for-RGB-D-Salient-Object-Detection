...
class data_loader():
    def __init__(self, data_path,  model= 'train' , subset=1, transform=None):
        self.data_path = data_path
        if model == 'train':
              self.path_file  = data_path+'split/train'+str(subset)+'.txt'
        elif model == 'test':
              self.path_file = data_path+'split/test'+str(subset)+'.txt'
        else:
...
class RandomAdjust(object):
    def __call__(self, sample):
        ...
        thermal = self.adjust_saturation(thermal)
        sample['image'] = image 
        sample['thermal'] = thermal 
        return sample
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        label32 = label.resize((w//32, h//32), Image.NEAREST)
        image = np.array(image).astype(np.float32).transpose((2, 0, 1)) / 255.0
        thermal = np.expand_dims(np.array(thermal), 0).astype(np.float32)/ 255.0
        ...
        label32  = np.expand_dims(np.array(label32), 0).astype(np.float32)/ 255.0
        ...
        label32 = torch.from_numpy(label32).float()
        return {'image': image,
                'thermal': thermal,
                'label': label,
                'label2': label2,
