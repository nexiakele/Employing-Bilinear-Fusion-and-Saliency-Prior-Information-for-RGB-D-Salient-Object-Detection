...
class data_loader():
    def __init__(self, data_path,  model= 'train' , subset=1, transform=None):
        ...
        if model == 'train':
            ...
            if subset == 1:
              self.path_file = data_path+'test_nju.txt'
            elif subset == 2:
              self.path_file = data_path+'test_nlpr.txt'
            elif subset == 3:
              self.path_file = data_path+'SIP_test.txt'
            elif subset == 4:
              self.path_file = data_path+'STEREO_test.txt'
            else:
                print('Test set not exsiting!!!!!!!!!!!!!!!!!!!!!!!!!!')
        else:
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        thermal = 1.0 - torch.from_numpy(thermal).float()
