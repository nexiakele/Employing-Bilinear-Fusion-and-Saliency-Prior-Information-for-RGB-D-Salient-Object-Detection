...
import tools.nju_load as data_loader
from tools.metrics import Eval_tool
#############################################
from model import  get_model
import Loss as ls
import Config
import modules.layers as _layer
import torch.nn as nn
#############################################
class models(torch.nn.Module):
    def __init__(self):
        super(models, self).__init__()
        self.conv1 = _layer.conv3x3xbnxrelu(4,32,2)
        self.conv2 = _layer.conv3x3xbnxrelu(32,32,2)
        self.conv3 = _layer.conv3x3xbnxrelu(32,32,2)
        self.dconv1 = _layer.basic_deconv_up(32, 32)
        self.dconv2 = _layer.basic_deconv_up(32, 32)
        self.dconv3 = _layer.basic_deconv_up(32, 32)
        self.p = nn.Sequential(_layer.conv1x1(32,1), nn.Sigmoid())
...
def ckpt_load(args, net, optimizer, device):
    if args.last_ckpt is not '':
          ...
          val_record2 = np.load(summary_dir + '/val_record2.npy').tolist()
    else:
          #不使用预训练的模型，则创建新目录
          ...
          val_record   = []  
          val_record2   = [] 
    return train_report, val_record,val_record2, summary_dir, ckpt_dir, time_dir                
...
###############################################################################
###############################是否使用预训练模型################################
    train_report, val_record,val_record2, summary_dir, ckpt_dir, time_dir = ckpt_load(args, net, optimizer, device)
##############################开始训练##########################################
    print('==================>开始训练<==================')
    eval_tool = Eval_tool()
...
#    train()
    x=torch.rand(4,3,32,64) #随便定义一个输入
    y=torch.rand(4,1,32,64) #随便定义一个输入
    gt = torch.ones(4,1,32,64)
    m = models()
    out = m.forward(x, y)
