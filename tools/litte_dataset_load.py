...
class data_loader():
    def __init__(self, data_path,  model= 'test' , subset='SSD', transform=None):
        self.data_path = data_path + subset +'/'
        if  model == 'test':
              self.path_file = data_path+ subset+'/test.txt'
        else:
              print('model not exist!')
        self.path = np.loadtxt(self.path_file, dtype=str)
        self.transform = transform
        print(self.path_file, len(self.path))
    def __len__(self):
